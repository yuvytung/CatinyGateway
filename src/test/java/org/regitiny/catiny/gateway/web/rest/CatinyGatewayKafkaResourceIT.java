...
import org.regitiny.catiny.gateway.config.KafkaProperties;
...
class CatinyGatewayKafkaResourceIT
{
  ...
  void setup()
  {
    ...
    CatinyGatewayKafkaResource kafkaResource = new CatinyGatewayKafkaResource(kafkaProperties);
  ...
  {
    restMockMvc.perform(post("/api/catiny-gateway-kafka/publish/topic-produce?message=value-produce"))
  ...
  void consumesMessages() throws Exception
  {
    ...
    MvcResult mvcResult = restMockMvc.perform(get("/api/catiny-gateway-kafka/consume?topic=topic-consume"))
      .andExpect(status().isOk())
